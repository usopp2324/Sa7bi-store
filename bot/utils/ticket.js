...
function buildOrderContainer(order) {
  ...
  const discordUser = order.discord_tag 
    ? order.discord_tag 
    : `<@${order.discord_id}>`;

  const rawThumbnailUrl = items.find((item) => item && item.image_url)?.image_url;
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'Use $paid <order_id> to confirm payment' });
